#!/bin/sh
# ChatGPT launcher that honours per-user Chromium/Electron flags.
# Flags are read from ${XDG_CONFIG_HOME:-~/.config}/chatgpt-flags.conf, one per
# line, with blank lines and '#' comment lines ignored. Original CLI arguments
# are appended after the configured flags.
